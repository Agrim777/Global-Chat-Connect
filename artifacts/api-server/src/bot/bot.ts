...
const BUTTONS = {
  ...
  report: "🚩 Report User",
  deals: "🔥 Amazon Deals",
  ebook: "📘 How to Make a Bot and Earn",
  edit: "✏️ Edit Profile",
...
async function sendEbook(chatId: number) {
  await bot.sendMessage(chatId, "📘 <b>How to Make a Bot and Earn</b>\n\nThis Ebook is the fastest and most practical guide to teach you how to earn money by creating a bot.\n\nResults depend on your skills, effort, and market — no income is guaranteed.", {
    parse_mode: "HTML",
    disable_web_page_preview: false,
...
bot.on("message", async (msg) => {
  ...
  if (user.state === "chatting") {
    ...
    // Main-menu buttons must never be relayed as chat messages.\n    // End the active chat cleanly, then run the selected menu action.\n    const menuSelections = new Set([\n      "find a match", "find match", "match", "find",\n      "how to make a bot and earn", "ebook",\n      "my profile", "edit profile", "delete account",\n      "unlock premium", "go premium", "premium",\n      "amazon deals", "deals", "create profile",\n      "match female", "match females", "match male", "match males", "admin panel",\n    ]);\n    if (menuSelections.has(action)) {\n      await disconnect(id, "The anonymous chat ended because you selected a menu option.");\n      const freshUser = await getUser(id);\n      if (!freshUser) return;\n      if (action === "find a match" || action === "find match" || action === "match" || action === "find") {\n        await findMatch(id, msg.chat.id);\n      } else if (action === "how to make a bot and earn" || action === "ebook") {\n        await sendEbook(msg.chat.id);\n      } else if (action === "edit profile") {\n        await showEditMenu(msg.chat.id);\n      } else if (action === "delete account") {\n        await bot.sendMessage(msg.chat.id, "⚠️ Delete your account? Use the confirmation below.", { reply_markup: { inline_keyboard: [[{ text: "✅ Yes, delete my account", callback_data: "delete_account_confirm" }], [{ text: "↩️ Keep my account", callback_data: "delete_account_cancel" }]] } });\n      } else if (action === "unlock premium" || action === "go premium" || action === "premium") {\n        await sendPremium(msg.chat.id);\n      } else if (action === "my profile") {\n        await bot.sendMessage(msg.chat.id, "Use /profile to view your profile. Gender cannot be changed after signup.");\n      } else if (action === "amazon deals" || action === "deals") {\n        await sendDeals(msg.chat.id);\n      } else if (action === "create profile") {\n        await startProfile(msg.chat.id, id);\n      } else if (id === ADMIN_ID && (action === "match female" || action === "match females")) {\n        await findMatch(id, msg.chat.id, "female");\n      } else if (id === ADMIN_ID && (action === "match male" || action === "match males")) {\n        await findMatch(id, msg.chat.id, "male");\n      } else if (id === ADMIN_ID && action === "admin panel") {\n        await bot.sendMessage(msg.chat.id, "🛠️ <b>Admin controls</b>\n\n/ban ID reason\n/unban ID\n/grantlifetime ID\n/stats", { parse_mode: "HTML", reply_markup: mainKeyboard(freshUser) });\n      } else {\n        await sendMain(msg.chat.id, freshUser);\n      }\n      return;\n    }\n    if (isScamContactRequest(text)) { await sendScamWarning(id, user.chattingWith || undefined, text); }
...
// Admin: broadcast the Ebook promotion to reachable, opted-in users.\nbot.onText(/^\/broadcastebook$/i, async (msg) => {\n  if (!ADMIN_ID || msg.from?.id !== ADMIN_ID) return;\n  if (broadcastOfferRunning) {\n    await bot.sendMessage(msg.chat.id, "⏳ Another broadcast is already running. Please wait for its delivery summary.").catch(() => {});\n    return;\n  }\n  broadcastOfferRunning = true;\n  const startedAt = Date.now();\n  let sent = 0;\n  let skipped = 0;\n  let blocked = 0;\n  const failureExamples: string[] = [];\n  try {\n    await bot.sendMessage(msg.chat.id, "🚀 Ebook broadcast started.\n\nSending it to all non-banned users who have not opted out. Blocked or deleted chats will be skipped automatically.").catch(() => {});\n    await schemaReady;\n    const allUsers = await db.select({ id: usersTable.id }).from(usersTable).where(and(\n      eq(usersTable.isBanned, false),\n      eq(usersTable.broadcastOptOut, false),\n    ));\n    const message = "📘 <b>How to Make a Bot and Earn</b>\n\n" +\n      "This Ebook is the fastest and most practical guide to teach you how to earn money by creating a bot.\n\n" +\n      "💡 Learn, build, test, and grow your skills. Results depend on your effort and execution — no income is guaranteed.\n\n" +\n      "Tap below to learn more.";\n    for (const target of allUsers) {\n      if (target.id === ADMIN_ID) continue;\n      if (sent + skipped > 0) await wait(350);\n      try {\n        await bot.sendMessage(target.id, message, {\n          parse_mode: "HTML",\n          disable_web_page_preview: false,\n          reply_markup: { inline_keyboard: [[{ text: "📖 Open Ebook", url: "https://confidentlife.gumroad.com/l/wrxspb" }]] },\n        });\n        sent++;\n      } catch (err) {\n        skipped++;\n        if (isBlockedTelegramChatError(err)) blocked++;\n        const errorMessage = err instanceof Error ? err.message : String(err);\n        if (failureExamples.length < 5) failureExamples.push(String(target.id) + " : " + errorMessage.slice(0, 120));\n        logger.warn({ userId: target.id, err }, "Ebook broadcast skipped unavailable Telegram chat");\n      }\n    }\n    const eligibleUsers = allUsers.filter((target: { id: number }) => target.id !== ADMIN_ID).length;\n    const durationSeconds = Math.max(0, Math.round((Date.now() - startedAt) / 1000));\n    const details = failureExamples.length ? "\n\nFirst failures:\n" + failureExamples.map((item) => "• " + item).join("\n") : "";\n    await bot.sendMessage(msg.chat.id, "✅ Ebook broadcast completed.\n\n📨 Sent: " + sent + "\n🚫 Blocked/deleted chats: " + blocked + "\n⚠️ Other skipped/failed: " + Math.max(0, skipped - blocked) + "\n👥 Eligible accounts: " + eligibleUsers + "\n⏱️ Time taken: " + durationSeconds + " seconds" + details).catch(() => {});\n    logger.info({ sent, skipped, blocked, eligibleUsers, durationSeconds }, "Ebook broadcast completed");\n  } catch (err) {\n    const durationSeconds = Math.max(0, Math.round((Date.now() - startedAt) / 1000));\n    logger.error({ err, durationSeconds }, "Ebook broadcast failed");\n    await bot.sendMessage(msg.chat.id, "❌ Ebook broadcast stopped after " + durationSeconds + " seconds due to a server or database error.").catch(() => {});\n  } finally {\n    broadcastOfferRunning = false;\n  }\n});\n// Admin: manually retry the recent-payer lifetime gift run.
